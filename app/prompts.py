...
사용자 요청에 맞는 첫 번째 메인 페이지(index.html) 전체를 완전한 HTML로 생성하세요.

## 규칙:
1. 반드시 `===HTML_START===` 로 시작하여 `===HTML_END===` 로 끝내세요. 코드 블록(```html)을 사용하지 마세요.
2. 네비게이션 메뉴 목록: {menu_items}
3. 메인 네비게이션 바(헤더)와 푸터를 필수로 포함시키세요.
4. 네비게이션 바의 링크는 계획된 파일 경로를 사용하세요. (예: 홈 -> "index.html", 소개 -> "pages/about.html" 등)
...
4. **디자인 일관성**: 메인 페이지의 `<head>` 내 `<style>` CSS 코드와 폰트, 컬러 스키마, 클래스명을 **그대로 사용/복제**하여 메인 페이지와 완벽히 동일한 디자인 톤앤매너를 유지하세요.
