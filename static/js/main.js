...
function updatePreview(html, isStreaming) {
  ...
  try {
    ...
    // 재렌더 후 선택 요소 재선택 (x로 취소 전까지 선택 유지)
    frame.onload = function () {
      try {
        const sel = state.selectedElement;
        if (state.devMode && sel && sel.wgen_id && frame.contentWindow) {
          frame.contentWindow.postMessage({ type: "reselect", wgenId: sel.wgen_id }, "*");
        }
      } catch (_) {}
    };
    frame.src = url;
...
async function sendMessageV2(message, displayMessage, elementContextObj, forcedMode) {
  ...
  try {
    ...
    // x로 취소하기 전까지 선택 유지 (삽입/링크액션 모드만 1회성 해제)
    if (!state.selectedElement || state.selectedElement.insertMode || state.pendingElementAction) {
...
async function routeByIntent(message, displayMessage, elInfo) {
  ...
  if (elInfo && !_wantsWhole) {
    ...
    // -0.6) 크기(너비/폭/높이/길이) Npx|% → 결정적 스타일 (AI 미사용)
    if (/(너비|폭|width|높이|height|길이)/i.test(message)) {
      const _sizeNear = (kw) => {
        const mm = message.match(new RegExp("(?:" + kw + ")\\s*(?:를|을|:|=|은|는)?\\s*(\\d+(?:\\.\\d+)?)\\s*(px|rem|em|%|vw|vh)?", "i"));
        return mm ? mm[1] + (mm[2] || "px") : null;
      };
      const styles = {};
      const w = _sizeNear("너비|폭|width|가로|길이");
      const h = _sizeNear("높이|height|세로");
      if (w) styles["width"] = w;
      if (h) styles["height"] = h;
      if (!Object.keys(styles).length) {
        const vM = message.match(/(\d+(?:\.\d+)?)\s*(px|rem|em|%|vw|vh)?/i);
        if (vM) styles["width"] = vM[1] + (vM[2] || "px");
      }
      if (Object.keys(styles).length) {
        if (styles["width"]) styles["max-width"] = "100%";
        console.log("[intent] size", styles);
        await execElementPatch({ op: "style", styles, also: _also }, elInfo);
        return;
      }
    }
...
window.addEventListener("message", function (e) {
  ...
  if (d.type === "element-selected") {
    ...
    if (!d.insertMode) {
      info += `\n\n📏 **크기·여백**: "마진 20px", "패딩 16px"(상하/좌우/상단·하단 지정 가능), "너비 300px", "높이 200px", "길이 50%".`;
      info += `\n\n👥 **다른 요소도 같이**: "이거랑 버튼도 빨갛게", "제목이랑 같이 가운데" 처럼 다른 요소를 지명하면 함께 변경됩니다.`;
    }
    info += `\n\n⌨️ **Delete 키**로도 선택한 요소를 바로 삭제할 수 있어요. (선택은 ✕ 누를 때까지 유지)`;
    info += `\n\n🖱 **Shift 또는 Ctrl + 클릭**으로 여러 요소를 한 번에 선택해 같이 삭제할 수 있어요.`;
    addMessage("messages", "assistant", info);
    scrollToBottom("messages");
  }
  ...
  if (d.type === "elements-multi-selected") {
    ...
    if (n === 0) { hideSelectedElementBar(); el.userInput.placeholder = "회사 소개, 서비스, 강조 포인트를 입력하세요..."; }
      ...
      const bar = document.getElementById("selected-element-bar");
      const label = document.getElementById("selected-element-label");
      if (bar && label) { label.textContent = `🗂 ${n}개 요소 선택됨 (Shift/Ctrl+클릭으로 추가)`; bar.classList.remove("hidden"); }
      el.userInput.placeholder = `${n}개 선택됨 — Delete 키 또는 "삭제" 입력으로 모두 삭제`;
    }
  }
